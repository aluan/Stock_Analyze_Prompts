# 静态价值型烟蒂股量化分析 Prompt v1.5

> **版本说明**：v1.5 — 基于v1.4，新增两项逻辑修正：(1) 子类型判定引入硬性门槛机制，核心条件不满足则该类型不成立，禁止模糊变通；(2) Fact Check评级引入警告分类机制（Data型 vs Risk型），仅Risk型警告计入评级，解决数据不足场景下评级偏严的问题。
...
2. **财报数据提取**：
   ...
   - **[v1.5] PDF数据不足时的补充机制**：当用户提供的PDF财报中无法找到所需数据（如FCF历史、完整股息记录、经营性现金流等），必须按以下优先级主动补充：
...
> 双标签机制：当A型标的的持股价值覆盖率 >= 50% 时，应同时标记为"A+B"混合类型，需额外完成子类型B中的SOTP分析和折价敏感性测试。前提：A型核心条件（股息率/PB/连续派息）必须全部满足，否则不得标记A+B。
...
- **[v1.5] 硬性门槛约束**：严禁对核心条件不满足的类型执行专项评估或给予双标签。例如：股息率低于市场门槛 -> A型不成立 -> 不执行A型评分 -> 不可标记A+B。若某类型不成立但其相关数据具有参考价值（如股息历史），可在报告中以"参考附录"形式呈现，但不得作为正式子类型判定依据。

### Step 7: Fact Check 21项验证
- 逐项完成#1-#19核心检查，标注「PASS / WARNING / VETO」
- **[v1.5] 对每个WARNING项进行分类**：判定为WARNING-Data（数据缺失型）还是WARNING-Risk（实质风险型），并在结果列标注分类
- 完成#20-#21加分项核查
- 完成对应市场的特殊风险检查
...
> **[v1.5] 判定铁律**：每个子类型的核心条件必须**全部满足**才能判定为该类型。任一核心条件不满足，该类型**不成立**，禁止使用"边缘""接近""部分满足"等模糊表述来变通。不成立的类型不得参与双标签组合。
...
B型条件（三项全部满足才成立）：
  ...
  2. 持股价值覆盖率 >= 30%? -> {PASS/FAIL}
  3. 母公司净现金 > 0? -> {PASS/FAIL}
