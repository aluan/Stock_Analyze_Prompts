# 静态价值型烟蒂股量化分析 Prompt v1.6

> **版本说明**：v1.6 — 基于v1.5，主要变更：(1) 修复T2公式遗漏短期理财/定期存款；(2) 加分体系（#20/#21）统一定义一次、各子类型引用，消除6处重复；(3) 修复报告模板编号错乱（支柱三7.1/7.2、季度复查12.3、数据来源13.1/13.2）；(4) B型筛选条件明确"持股比例≥10%"与"覆盖率≥30%"为两个独立必要条件；(5) C2评分新增"估值安全垫≥1分"硬性门槛；(6) 治理评分去除无效权重列；(7) 补充边界条件（负净资产、双重股权、币种换算、财年对齐、中报年化）；(8) 新增[COMPUTABLE]/[LLM-REQUIRED]标签，便于平台代码化；(9) 与QuantRadar平台顶层规划对齐监控频率与数据流。
...
5. **[v1.6] 标签体系**：策略知识库中的量化指标标注了以下标签，用于指导平台代码化分工：
...
> **[v1.6] T2公式修正说明**：v1.5中T2公式仅列出"现金等价物×1.0"，遗漏了短期理财和定期存款。实际上T2是对全部流动资产打折，现金类资产（含短期理财+定期存款）应以1.0系数计入，与T0/T1的现金池口径保持一致。
...
> **[v1.6] 结构调整**：加分体系在v1.5中分散于支柱一、子类型A/B/C、Fact Check等多处重复定义。v1.6将其统一定义于此，各子类型章节仅说明适用方式，不再重复规则。
...
> **[v1.6] 修正说明**：v1.5中B型必要条件2表述为"子公司持股比例≥10%"，但支柱三总览表中还列出了"覆盖率≥30%"。v1.6明确两者为独立的必要条件，均须满足。"持股比例≥10%"确保有实质影响力/控制权，"覆盖率≥30%"确保持股价值对母公司市值有足够的撬动效应。
...
> **[v1.6] C2评分硬性门槛**：即使总分≥6分，若"估值安全垫"维度得分为0分（NAV/市值<1.5x），则C2不达标。理由：C2是"政策期权"策略，没有估值安全垫意味着即便政策兑现，上行空间也不足以补偿等待成本和政策落空风险。
...
> **[v1.6] 与QuantRadar平台对齐**：以下监控频率与平台调度引擎的设计一致（月度/季度/半年度/年度四档）。
...
5. **[v1.6]** 确认财年起止月份；如使用中报数据，标注年化处理方式
...
> **[v1.6] 评分表修正**：v1.5中包含"权重"列，但评分体系为直接求和（非加权），权重列实际不参与计算。v1.6移除该列以避免误导。各维度的满分分值本身已隐含重要性区分（持股绑定3分 > 沟通透明2分）。
